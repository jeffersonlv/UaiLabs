...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h4 class="mb-0">{{ $unit->exists ? 'Editar Unidade' : 'Nova Unidade' }} — {{ $company->name }}</h4>
    <a href="{{ route('admin.units.index', $company) }}" class="btn btn-outline-secondary btn-sm">Voltar</a>
</div>

<div class="card border-0 shadow-sm">
    <div class="card-body">
        <form method="POST" action="{{ $unit->exists ? route('admin.units.update', [$company, $unit]) : route('admin.units.store', $company) }}">
            @csrf
